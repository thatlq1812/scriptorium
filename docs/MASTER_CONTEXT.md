...
| 1.12.0 | 2026-08-15 | Claude | thatlq1812 directed renaming the root `outside_research/` living-input directory to `references/` (the `.gitignore` rule `/references` already existed from 2026-08-13's `168272c`, added ahead of this rename). Updated the repo-structure tree and the "living input, not history" convention paragraph in this file, plus the matching current-state descriptions in `CLAUDE.md`, `README.md`, `docs/README.md`, `docs/ROADMAP.md`'s "Long-term direction" section, and `docs/specs/STRATEGY_SPEC.md` principle 4. Deliberately did NOT touch specific-file historical citations (`registry/skills.json` `elicited_from` fields, dated changelog rows across `ROADMAP.md`/`STRATEGY_SPEC.md`, `docs/STATUS.md`, `docs/templates/CLUSTER_SURVEY_TEMPLATE.md`, `skill-creator`'s `outside_research/references/book-to-skill` provenance citation) — those record what file was actually used under its then-current name at build time; renaming them would misrepresent provenance, same discipline as this repo's immutable-`skill_id`/never-rewrite-`docs/archive/` conventions. `outside_agy/` is unaffected, kept as its own separate gitignored directory. Empty `references/` directory created locally (gitignored, nothing to migrate — `outside_research/` never held committed or on-disk content). |
