...
| Last Updated | Status |
| --- | --- |
| 2026-07-26 | 18 skills written. All have passed `security-audit` (all `passed`), none have a `quality_score` yet (stage 4 hasn't run on any skill) — so no skill is officially "ready to use" yet, even though several have already seen real use this session. Harvesting the "general tier" from K-Dense-AI/scientific-agent-skills (MIT) is complete this round: `citation-management`, `literature-review`, `exploratory-data-analysis`, `hypothesis-generation`, `peer-review` all done. See `docs/ROADMAP.md` for the running expansion backlog. Everything below is verified directly against `registry/skills.json` + `skills/`. |
...
| `exploratory-data-analysis` | 0.1.0 | N1 | `null` | `passed` | **Not yet official** — third harvested skill (from K-Dense-AI/scientific-agent-skills, MIT). Bounded local CSV/TSV/JSON profiler (schema/missingness/IQR-outlier stats), stdlib-only, zero network calls, path-confined to `--root`. Scoped down to the original's "automated core" tier only (dropped optional NumPy/h5py/Biopython/Pillow/tifffile inspectors + 6 biology/chemistry reference docs as specialized-tier). Verified real: CSV profile (6 rows, caught a real IQR-fence outlier + 2 missing values), JSON profile (list-of-dicts, correct field/type/missing detection), 3/4 safety rejections verified for real (outside-root, nonexistent file, unsupported extension) — symlink rejection uses the same trusted `Path.is_symlink()` pattern but couldn't be empirically re-verified in this sandbox (no admin rights to create a real symlink). Hasn't passed stage 4. |
| `hypothesis-generation` | 0.1.0 | N2 | `null` | `passed` | **Not yet official** — fourth harvested skill (from K-Dense-AI/scientific-agent-skills, MIT). Turns an observation into rival candidate hypotheses + discriminating predictions + a preregistration scaffold; hypothesis status can never be "confirmed" by construction. 3 scripts (schema validator, causal-claim linter, preregistration generator), stdlib-only, zero network. Scoped down from the original's 7-script suite. Verified real: valid record passed, a deliberately broken record correctly caught all 4 errors (single candidate, `status=confirmed`, dangling prediction reference, missing accountable_human), linter correctly flagged 5 overclaims and passed a hedged rewrite clean, preregistration scaffold generated correctly. N2 reflects subject-matter risk, not a tooling finding. Hasn't passed stage 4. |
| `peer-review` | 0.1.0 | N2 | `null` | `passed` | **Not yet official** — fifth and final general-tier harvested skill this round (from K-Dense-AI/scientific-agent-skills, MIT). Mandatory intake gate (authorization/confidentiality/accountability) blocks all manuscript inspection until `READY_FOR_LOCAL_REVIEW`; claim-evidence matrix validator; private review scaffold with strict Comments-to-Authors/Confidential-Comments-to-Editor channel separation; tone/placeholder/decision-overreach linter. 4 scripts, stdlib-only, zero network. Scoped down from the original's 7-script suite. Verified real: valid intake reached ready state, a broken intake correctly triggered all 7 blocking conditions, claim matrix validated (after fixing a real CSV-quoting bug found during testing), a broken matrix caught 5 errors, scaffold generator correctly refused an unauthorized intake, linter passed a clean review and flagged a missing section + abusive language + a decision-overreach phrase in a violating draft. N2 reflects subject-matter risk, not a tooling finding. Hasn't passed stage 4. |
