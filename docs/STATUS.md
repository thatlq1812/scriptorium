...
| Last Updated | Status |
| --- | --- |
| 2026-07-26 | 17 skills written. All have passed `security-audit` (all `passed`), none have a `quality_score` yet (stage 4 hasn't run on any skill) — so no skill is officially "ready to use" yet, even though several have already seen real use this session. Harvesting the "general tier" from K-Dense-AI/scientific-agent-skills (MIT) is in progress: `citation-management`, `literature-review`, `exploratory-data-analysis`, `hypothesis-generation` done; `peer-review` remaining. See `docs/ROADMAP.md` for the running expansion backlog. Everything below is verified directly against `registry/skills.json` + `skills/`. |
