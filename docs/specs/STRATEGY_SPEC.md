...
| 1.5.0 | 2026-07-26 | Claude | Owner yêu cầu chuyển venv từ per-skill sang venv DÙNG CHUNG ở root repo — §7 điểm 7 cập nhật. `python-env-bootstrap` v0.2.0 quản lý venv chung, verify thật (3 skill Python cùng cài, import chéo không xung đột). |
...
7. Không bao giờ commit venv/binary environment vào git — venv gắn OS/kiến trúc cụ thể, phá portability. Skill cần Python khai báo `requirements.txt` (hoặc lockfile tương đương) + hướng dẫn bootstrap venv tại runtime; `.venv/` luôn nằm trong `.gitignore`. **Sửa 2026-07-26 (owner)**: venv KHÔNG nằm riêng trong từng skill nữa — một venv DÙNG CHUNG duy nhất ở root repo (`<repo_root>/.venv`, sibling `skills/`), tránh trùng lặp dependency nặng qua nhiều skill. Skill hạ tầng `python-env-bootstrap` (dựa trên `uv`) quản lý venv chung này — mọi skill Python mới phụ thuộc vào nó qua `registry` field `dependencies`, gọi `bootstrap.ps1/.sh -Requirements <skill>/requirements.txt` để thêm dependency của mình vào venv chung thay vì tạo venv riêng.
