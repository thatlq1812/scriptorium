...
| --- | --- | --- |
| Student (K12) | **Owner-confirmed as of 2026-07-26.** No independent student survey exists, but the owner explicitly confirmed `outside_research/` + direct instruction count as elicitation for this tier — Claude (acting as the skill-creator sub-agent) builds directly from it, scoped conservatively. **2026-07-27**: real secondary research done (`outside_research/research_02/research_02_result_01.md`) — confirmed EduStation has zero student-facing precedent (re-checked all 63 folders), found real 2025-2026 data quantifying the "doing the student's work" risk (College Board/PEW/faculty surveys, arXiv:2605.21629), a real design pattern that resolves it (Khanmigo's Socratic-restraint), and a real curriculum hook (CT GDPT 2018's "năng lực tự chủ và tự học"). Still candidate ideation, not elicited input — no real student/parent/teacher survey exists yet. | **In progress** — `study-plan-builder` (v0.1.0) done: deterministic study/review scheduling, deliberately avoids any subject-matter content to sidestep the "doing the student's work" risk while real per-topic elicitation is still thin. Research brief above identifies a safe, non-elicitation-gated next step (spaced-review extension of `study-plan-builder`) and flags a high-value/high-risk one (Socratic concept-helper) that still needs a real survey before `skill-creator` runs. |
