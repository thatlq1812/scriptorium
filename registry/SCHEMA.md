...
| `quality_score` | object \| null | Result from the quality-eval loop (pipeline step 4). Four-field structure: `{ "stage4_formal": object \| null, "content_grounding": string, "stage4_required": boolean, "stage4_reason": string }`. `stage4_formal` is `null` until stage 4 has actually run. `content_grounding` is one of: `"auto-verified"` (skill built by pipeline crew against authoritative public sources — official curriculum standards, peer-reviewed literature, MIT-licensed prior art, real deployed systems — no external content review needed); `"partial"` (known content gaps explicitly flagged in STATUS.md); `"pending"` (content grounding not yet assessed). `stage4_required` (owner decision, 2026-07-29) scopes which skills the formal stage-4 gate actually applies to — **not** a blanket requirement across the registry: `true` only for (a) niche-specializer skills elicited from a real expert source (interview, real prior deployed system, practitioner survey — see `CLAUDE.md` principle 4's tier model), or (b) skills whose core function ingests uncontrolled external input (arbitrary documents, web content, third-party repos, external API responses — the injection/hallucination-risk surface stage 4's adversarial-input Pass A is built to catch). `false` for foundation/infrastructure and general-capability skills grounded in public sources with no uncontrolled external input — these are exempt from the formal gate by design, not merely deferred. `stage4_reason` is a short human-readable justification for the classification. Running stage 4 on any `stage4_required: true` skill is still deliberately not yet scheduled (see `CLAUDE.md`) — do not start without asking. **Do not read `stage4_formal: null` as "skill unverified"** — it means the formal multi-harness scoring gate has not been scheduled yet, regardless of `stage4_required`. |
...
| `operational_status` | object \| absent | Added 2026-07-29. Absent (or `{"state": "active"}`) is the default — no special handling. `{"state": "paused", "reason": string, "date": string, "decided_by": "owner"}` marks a skill that passed structural validation and security audit but is deliberately NOT recommended for production use pending a real owner-identified gap (e.g. a design premise later found not to hold, or a required real-elicitation source still missing) — distinct from `security_audit.status != "passed"` (a safety finding) and `license_debt` (a legal blocker). `{"state": "superseded", "superseded_by": string (skill_id), "reason": string, "date": string, "decided_by": "owner"}` (added 2026-08-07) marks a skill whose real capability has been replaced by a specific named successor skill (never renamed in-place — `skill_id` stays immutable per this file's own rule, so a genuine architecture/technology replacement gets a new `skill_id`, and the old one is marked superseded rather than deleted, same reversibility discipline as `paused`). `skill-exporter`'s hard-exclusion applies to both `paused` and `superseded` — either is refused from any export bundle, same "whole bundle or nothing" posture as the other 2 hard-exclusion rules. |
