...
description: 'Generates speech audio (TTS) from text using Gemini, via the user''s OWN API key -- optional, not an AI backend managed by Scriptorium. Supports single prebuilt-voice narration, 2-speaker dialogue (labelled script text + a voice per speaker), and voice cloning (a sample of the target voice PLUS a separate recorded consent clip -- this skill refuses to clone a voice without proof of consent, no override). Output is always wrapped into a proper .wav file (Gemini''s raw API response is unwrapped PCM, not a playable file on its own -- verified 2026-08-05). Use when the user already has a Gemini API key and needs narration/dialogue audio for a video, or a cloned voice with real consent already recorded. Do NOT use for music or sound-effect generation -- Gemini''s music model (Lyria) is only exposed as an experimental real-time WebSocket streaming session (`AsyncLiveMusic`), a fundamentally different, session-based API shape than this skill''s simple request/response TTS calls; deliberately NOT implemented here rather than half-built, see "What this skill does NOT do".'
license: MIT
...
```bash
uv pip install --python .venv -r skills/audio-generator-gemini/requirements.txt
```

## Single-speaker narration

```bash
.venv\Scripts\python.exe skills\audio-generator-gemini\scripts\generate_speech.py "Say cheerfully: Welcome to the show!" out.wav --voice Kore
```
...
```bash
.venv\Scripts\python.exe skills\audio-generator-gemini\scripts\generate_speech.py \
...
```bash
.venv\Scripts\python.exe skills\audio-generator-gemini\scripts\generate_speech.py "Hello, this is a cloned voice." out.wav \
