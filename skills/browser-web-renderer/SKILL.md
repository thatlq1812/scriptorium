...
```bash
.venv/bin/python skills/browser-web-renderer/scripts/check_browser.py
# Windows: .venv\Scripts\python.exe skills\browser-web-renderer\scripts\check_browser.py
```
...
```bash
# Windows (real PowerShell, not Git Bash -- same uv platform-detection caveat python-env-bootstrap documents):
.\skills\browser-web-renderer\scripts\install_browser.ps1
# macOS/Linux:
bash skills/browser-web-renderer/scripts/install_browser.sh
```

Installs the `playwright` package into the shared venv (via `python-env-bootstrap`), then downloads the Chromium binary (~300MB, separate from the pip package) via `python -m playwright install chromium`. This is a real network download — never run without confirming via `check_browser.py` first that something is actually missing.

## Render and extract

```bash
.venv/bin/python skills/browser-web-renderer/scripts/render_and_extract.py <url> [-o output.json] [--screenshot path.png] [--timeout-ms 15000]
```
