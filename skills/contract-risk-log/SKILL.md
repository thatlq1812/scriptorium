...
Start from `assets/batch_tabular_review_template.json`. The caller declares `documents` (list) and `columns` (list, whatever review categories actually apply — no fixed vocabulary, since no real owner-supplied diligence schema was available to elicit one from). Every `(document, column)` pair needs exactly one cell in `cells`, each with a `state`:
...
- Batch mode's schema is generic (adapted from an outside pattern, not elicited from a real owner-supplied diligence checklist) — if a real checklist becomes available later, this schema should be revisited against it rather than assumed already correct.
