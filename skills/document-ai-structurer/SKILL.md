...
3. Run the candidate scanner (regex heuristics only, explicitly unverified):
   ```bash
   .venv/bin/python skills/document-ai-structurer/scripts/suggest_legal_boundaries.py <output_dir>/full.md -o structure.json
   ```
...
5. Once `structure.json` reflects what you actually verified, mechanically build the output:
   ```bash
   .venv/bin/python skills/document-ai-structurer/scripts/apply_legal_structure.py <output_dir>/full.md structure.json <output_dir> [--overwrite]
   ```
...
```bash
.venv/bin/python skills/document-ai-structurer/scripts/build_catalog.py <corpus_root_dir> -o catalog.json
```
