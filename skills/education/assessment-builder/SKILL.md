...
metadata:
  ...
  elicited_from: "Consolidates 6 prior system skill folders that were all variations of the same 'generate an exam matrix + exam + answer key balanced by cognitive level' capability, differing only in output shape (prior deployed system skills/exam_builder, review_exam, sample_exam, question_bank, review_outline, tn_thpt_review — all SKILL.md read directly, plus exam_builder/scripts/build_dethi.py and its knowledge/ma_tran_de.md + knowledge/de_dap_an_format.md for the actual balancing-algorithm reference). Per thatlq1812 direction (2026-07-26), consolidated into fewer, more practical skills rather than kept as 6 near-duplicate folders. Domain knowledge kept: the 4-cognitive-level framework (Nhận biết/Thông hiểu/Vận dụng/Vận dụng cao) and its default 40/30/20/10 ratio (explicitly a generic convention per prior system's own knowledge/CURATION.md, not an official per-subject regulation), the largest-remainder (Hamilton apportionment) balancing method so counts/points always sum exactly, the MCQ invariant (exactly 4 distinct choices, exactly one valid answer), and the tn_thpt_review skill's QD 4068/QD-BGDDT THPT-graduation-exam profile concept (folded in here as a named --profile preset, not a separate skill, per thatlq1812's explicit instruction). Domain knowledge deliberately DISCARDED: prior system's harness-specific orchestration machinery in every one of the 6 SKILL.md files (persona/effort/token_budget tuning, use_skill/task_create/script_exec tool-call conventions, Jinja2 {{ user.* }}/{{ institution.* }} profile placeholders, workspace-scan-before-work digest conventions, planning-gate dialogue scripts) — none of that is domain knowledge, all of it is infrastructure for prior system's own agent runtime. Also discarded: question_bank's per-question stable-code convention (<MON><khoi>-<chu de>.<muc>.<so thu tu>) and coverage-only (no total-point-balance) framing, tn_thpt_review's 3-answer-format (TN co dien + Dung/Sai + Tra loi ngan) QD 4068 structure and its per-subject ti_le_dang ratios — both are real domain knowledge but out of scope for a v0.1.0 that only covers the classic 4-level/MCQ+essay case; noted as a known limitation below for a possible v0.2+."
  version: 0.1.0
...
- **Default ratio 40/30/20/10** (Nhận biết/Thông hiểu/Vận dụng/Vận dụng cao) — per prior system's own `knowledge/ma_tran_de.md` and `knowledge/CURATION.md`, this is a **generic convention**, not an official per-subject/per-grade regulation. Always overridable via `level_ratio` in the input.
...
- Doesn't implement question-bank-style stable question codes/coverage tracking (prior system's `question_bank` skill) or a bộ-đề (exam-set)/multi-exam batch mode (prior system's `review_exam`) — a single matrix + a single exam per run in v0.1.0.
...
- **No stable per-question code / coverage-tracking mode.** prior system's `question_bank` skill tagged each question with a stable code (`<MÔN><khối>-<chủ đề>.<mức>.<số thứ tự>`) for cross-exam reuse and tracked coverage independent of point-balance. This skill only produces one point-balanced exam per plan; reusable tagged banks are out of scope for v0.1.0.
- **No multi-exam batch mode.** prior system's `review_exam` balanced N exams in one call (a bộ đề). This skill balances one matrix at a time — call `build_exam_matrix.py` once per exam in a set; there is no batch/index.json convenience layer.
