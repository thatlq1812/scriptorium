...
# Matches a field whose ENTIRE value is still wrapped in <angle brackets> --
# i.e. assets/lesson_plan_template.json's own instructional placeholder text
# (e.g. "<lesson/topic title>"), never replaced with real content. Anchored
# to the whole field, not a bare substring, so it never flags real lesson
# content that legitimately contains "<"/">" as inequality symbols (Toan
# content like "x < 5 va y > 3" does not fullmatch this pattern).
...
def validate(plan: dict) -> tuple[list[str], list[str]]:
    ...
    for field in ("mon_hoc", "ten_bai"):
        if is_placeholder(plan[field]):
            errors.append(f"{field} still contains unedited template placeholder text ({plan[field]!r}) -- replace with real content, never leave assets/lesson_plan_template.json's own placeholder in a submitted plan")
    ...
    elif is_placeholder(mt["kien_thuc"]):
        errors.append(f"muc_tieu.kien_thuc still contains unedited template placeholder text ({mt['kien_thuc']!r}) -- describe the lesson's own content in real words, never paste the placeholder or textbook/SGK text verbatim")
    ...
    for nl in nang_luc_dac_thu:
        if is_placeholder(nl):
            errors.append(f"muc_tieu.nang_luc_dac_thu contains unedited template placeholder text ({nl!r}) -- replace with a real subject-specific competency")
        elif nl not in KNOWN_NANG_LUC_DAC_THU:
    ...
    for i, activity in enumerate(hoat_dong):
        ...
        for field in ("muc_tieu", "noi_dung", "san_pham"):
            ...
            if not value:
                ...
                errors.append(f"hoat_dong[{i}] ({name}): {field} still contains unedited template placeholder text ({value!r}) -- describe this activity's own content in real words, never paste the placeholder or textbook/SGK text verbatim")
        ...
        if not isinstance(steps, dict):
            ...
            for step in REQUIRED_STEPS:
                ...
                elif is_placeholder(step_value):
                    errors.append(f"hoat_dong[{i}] ({name}): to_chuc_thuc_hien.{step} still contains unedited template placeholder text ({step_value!r}) -- replace with real content")
    ...
    for j, item in enumerate(plan.get("thiet_bi", [])):
        if is_placeholder(item):
            errors.append(f"thiet_bi[{j}] still contains unedited template placeholder text ({item!r}) -- replace with a real equipment/materials item")
    ...
    if is_placeholder(danh_gia):
        errors.append(f"danh_gia still contains unedited template placeholder text ({danh_gia!r}) -- replace with the real assessment approach")
