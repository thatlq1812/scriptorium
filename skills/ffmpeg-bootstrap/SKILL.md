...
description: 'Resolves a working `ffmpeg` binary for other skills to call via subprocess — PORTABLE-first, like every other bootstrap skill in this project: the binary lives inside this project''s own shared venv (`imageio-ffmpeg` bundles the real ffmpeg executable directly inside its pip wheel, no separate download step, no system-wide install, no dependency on whatever the host machine happens to have on PATH), with system PATH ffmpeg only as a last-resort fallback. `check_ffmpeg.py` detects (safe, zero network — the binary is already on disk once the pip package is installed), `resolve_ffmpeg.py` is the importable helper every other media skill in this project (`video-generator-gemini`, `audio-generator-gemini`, `video-assembly-composer`) calls to get the resolved path — never hardcodes `"ffmpeg"` and hopes it is on PATH. `install_ffmpeg_system.ps1`/`.sh` are OPTIONAL, secondary scripts for a real system-wide install (winget/brew/apt), only useful if ffmpeg is also wanted outside this project. Use before any skill in this project shells out to ffmpeg. Do NOT use this to actually transcode/edit/compose video or audio itself — it only resolves and verifies the binary path.'
license: MIT
