...
Point `<template.json>` at the workspace's own local copy (`workspace_root/projects/_templates/<template_id>/template.json`, printed by `init_workspace.py`) once the workspace is initialized. `projects_root` is the parent directory under which `<YYYYMMDD>-v<N>-<name>/` gets created (`workspace_root/projects/`, per `init_workspace.py`'s convention). `--name` (required, lowercase kebab-case, e.g. `hop-dong-internet`) is caller-supplied, never auto-generated -- pick a slug identifying the project, transliterating Vietnamese to plain ASCII. `<N>` is derived by scanning `projects_root` for existing directories ending in `-v<M>-<name>` (same `name`, any date) and taking `max+1` -- a brand-new name always starts at `v1`; re-scaffolding the same name later (a redo, a new round of the same matter) gets a new dated `v2`/`v3`/... directory instead of colliding. `--date` overrides today's date (useful for scripted testing or backfilling); omit it in normal use.
...
For when a single project is the whole unit of work -- no `projects/` root, no `WORKSPACE.md`, no `_templates/` copy, no date/sequence naming. The caller supplies `project_dir` directly (this script does not derive or validate a name) -- for consistency with `scaffold_workspace.py`'s convention, name it `<YYYYMMDD>-v<N>-<slug>` yourself when there's no stronger reason to name it otherwise. Creates the template's `subdirs` directly under `project_dir` and a `PROJECT.md` there. Refuses (exit 1) if `project_dir/PROJECT.md` already exists, unless `--force`. Reads only `subdirs`/`project_md_prompts` (same two fields `scaffold_workspace.py` reads) -- any template works in either mode unchanged.
...
`init_workspace.py` writes a fixed, profession-agnostic `AGENTS.md` alongside `WORKSPACE.md` -- not template-declared content, since these are rules about the workspace mechanism itself, not any one profession's workflow (same reasoning `WORKSPACE.md`'s own "Shared resources" section already follows). It covers: (1) the skill-registry lookup convention, (2) the `<YYYYMMDD>-v<N>-<name>` project naming convention with a real invocation example, (3) a mandatory rule that a request implying a deliverable gets a real file inside a scaffolded project, not a reply typed directly into the chat, (4) a rule to proactively keep `personals/profile.json` current via `personal-profile-manager` rather than treating it as a passive lookup, and (5) the shared-resources (skills dir, venv) reminder. Grounded in a real pilot-workspace gap review (see `metadata.changelog_0_4_0`) -- not invented from scratch.
...
- Version-number scanning only looks at direct children of `projects_root` matching the exact `<YYYYMMDD>-v<N>-<name>` pattern with an EXACT `--name` match -- a differently-named or differently-spelled existing directory for the same conceptual project is invisible to it (by design: never guess an existing project is "the same one"; a typo'd `--name` starts a fresh `v1` history instead of continuing the real one).
