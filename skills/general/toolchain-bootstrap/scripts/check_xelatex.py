...
anything itself; that's install_xelatex.ps1/.sh, run only when explicitly
...
INSTALL_HINTS = {
    "Windows": "winget install --id MiKTeX.MiKTeX -e   (or run install_xelatex.ps1)",
    "Darwin": "brew install --cask mactex-no-gui   (or run install_xelatex.sh)",
    "Linux": "sudo apt-get install -y texlive-xetex texlive-lang-vietnamese biber   (or run install_xelatex.sh)",
}
