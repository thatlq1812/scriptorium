...
Consolidates three real previously-deployed EduStation skills that all sit around the same scoring/feedback moment — grading (MCQ + essay), primary_remarks (Tiểu học qualitative remarks under TT27), and report_card_remarks (THCS/THPT qualitative remarks under TT22) — into one skill for the Teacher tier, per the owner's explicit direction to consolidate EduStation's ~63 over-fragmented skill folders into fewer, more practical ones. EduStation's own SKILL.md files packed in a large amount of harness-specific orchestration (persona/effort/token_budget tuning, batched sub-agent dispatch, Jinja2 profile placeholders, workspace-scan-before-work, `use_skill`/`script_exec`/`llm_call` tool calls) that is infrastructure for their own agent runtime, not domain knowledge — none of that was ported.
...
Per owner direction, this tier deliberately excludes EduStation's bureaucratic-paperwork skills (Nghị định 30/2020 administrative templates) — this skill serves the actual scoring/feedback work, not administrative process.
