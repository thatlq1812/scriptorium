...
skills/media-anchor-profile/SKILL.md):
...
def _load_anchor_profile_kwargs(profile_path: Path) -> dict:
    ...
    (.claude/skills/media-anchor-profile/), not a pip dependency of this one.
    ...
    loader_dir = skills_dir / "media-anchor-profile" / "scripts"
    ...
    try:
        ...
        sys.exit(
            ...
            f"sibling skill folder ({loader_dir} not found)."
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to a media-anchor-profile JSON file (see skills/media-anchor-profile/SKILL.md). "
