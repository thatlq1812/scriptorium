...
description: 'Ties `image-generator-gemini`, `video-generator-gemini`, `audio-generator-gemini`, and `video-assembly-composer` together into one topic -> script -> images -> videos -> audio -> assembled-video pipeline, driven by a single topic string. State/resume is embedded directly in the project''s own `project.json` (re-running skips whatever stage/scene is already done, never a separate checkpoint file that can drift out of sync with the actual artifacts on disk). Every Gemini API call (script/image/video/audio) is wrapped in an exponential-backoff retry. A human-approval checkpoint pauses the pipeline right after script generation, BEFORE any image/video/audio quota is spent, until the caller explicitly passes `--approve-script` -- catches a bad script cheaply instead of discovering it after paying for 4 generation stages. Use when the user wants a topic turned into a finished short video end-to-end with Gemini as the sole generation backend, or wants to resume a partially-completed run. Do NOT use this for one-off single-asset generation (a single image, a single video clip, a single voice line) -- call the individual skill directly for that; this skill''s value is the multi-stage state machine and cross-skill wiring, not any one generation call.'
license: MIT
compatibility: 'Requires Python 3.11+ + `google-genai` (shared venv) + the user''s own `GEMINI_API_KEY` + `image-generator-gemini`, `video-generator-gemini`, `audio-generator-gemini`, `video-assembly-composer`, and `ffmpeg-bootstrap` all installed as sibling skill folders (imported directly as Python modules, not invoked as subprocesses). Verified running clean: Claude Code, Windows (2026-08-05), a real 2-scene end-to-end run including a real script-generation checkpoint pause, resume-after-failure, and a real bug found and fixed during testing (see "A real constraint found by testing" below). See "Verified" below.'
metadata:
  domain: media
...
.venv\Scripts\python.exe skills\media-pipeline-orchestrator\scripts\run_pipeline.py my_project --topic "a fox's rainy day adventure" --n-scenes 2

# 2. Review my_project/scenes.json by hand (edit narration/visual_prompt/duration_seconds if needed), then:
.venv\Scripts\python.exe skills\media-pipeline-orchestrator\scripts\run_pipeline.py my_project --topic "a fox's rainy day adventure" --n-scenes 2 --approve-script
```
