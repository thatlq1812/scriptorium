...
registry (toolchain-bootstrap, browser-web-renderer...).
...
def main() -> int:
    ...
    if not up_to_date:
        ...
        known_ids = set(local_versions) | set(remote_versions)
        ...
        # A changed file's skill_id is whichever path component matches a known
        # skill_id -- NOT a fixed path index. This repo moved from flat
        # skills/<skill_id>/... to skills/<domain>/<skill_id>/... (2026-08-13);
        # during the transition the local clone may be nested while origin is
        # still flat (or vice versa), so a diffed path can have either depth.
        ...
        for line in changed_files:
            for part in line.split("/"):
                if part in known_ids:
                    found_ids.add(part)
                    break
        changed_skill_dirs = sorted(found_ids)
