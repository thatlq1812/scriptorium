...
def skill_dir_for(skill_id: str, registry: dict[str, dict]) -> Path:
    """Resolve a skill's real on-disk directory under skills/<domain>/<skill_id>/
    (repo-internal domain-per-folder layout, 2026-08-13) -- uses the registry's
    own domain tag first (authoritative), falls back to a one-level glob so a
    skill_id missing from this particular registry snapshot still resolves."""
    skill = registry.get(skill_id)
    domains = (skill.get("tags", {}).get("domain") if skill else None) or []
    if domains:
        candidate = SKILLS_DIR / domains[0] / skill_id
        if candidate.is_dir():
            return candidate
    found = next(SKILLS_DIR.glob(f"*/{skill_id}"), None)
    return found if found is not None else SKILLS_DIR / skill_id
...
def build_manifest(purpose: str | None, included: list[str], non_skill_deps: list[str], reasons: dict[str, str], registry: dict[str, dict]) -> str:
    ...
    if "toolchain-bootstrap" in included or non_skill_deps:
        lines += [
            "",
            "## Python environment",
            "",
            ...
            "time it's actually needed -- `toolchain-bootstrap`'s own `SKILL.md` documents the exact "
    ...
    lines += [
        ...
        "(e.g. `toolchain-bootstrap` for the shared Python venv, if a dependency above names it). "
...
def build_start_here(purpose: str | None, included: list[str], non_skill_deps: list[str]) -> str:
    ...
    needs_venv = "toolchain-bootstrap" in included or bool(non_skill_deps)
...
def main() -> int:
    ...
    lock = {
        skill_id: {
            ...
            "content_sha256": _content_sha256(skill_dir_for(skill_id, registry)),
        }
        for skill_id in included
    }
    ...
    with zipfile.ZipFile(args.output, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for skill_id in included:
            skill_dir = skill_dir_for(skill_id, registry)
            if not skill_dir.is_dir():
                print(f"REFUSED: {skill_id!r} is in the registry but not found on disk under skills/<domain>/{skill_id}/.", file=sys.stderr)
                return 1
            # Bundle output stays flat (skills/<skill_id>/...) regardless of this
            # repo's own internal domain-per-folder layout -- a consuming harness
            # (Claude Code, goose, Cursor...) has no concept of Scriptorium's
            # domain taxonomy and expects a flat skill_id directory.
            for path in skill_dir.rglob("*"):
                if path.is_dir() or "__pycache__" in path.parts:
                    continue
                arcname = Path("skills") / skill_id / path.relative_to(skill_dir)
