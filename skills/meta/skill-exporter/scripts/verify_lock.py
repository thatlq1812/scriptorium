...
def main() -> int:
    ...
    for skill_id, info in lock.items():
        if not isinstance(info, dict) or "content_sha256" not in info or "version" not in info:
            ...
            continue
        # Flat first (an extracted bundle's own skills/<skill_id>/ -- export_bundle.py
        # always writes bundles flat regardless of this repo's own internal domain
        # nesting); fall back to a one-level glob for this repo's own skills/<domain>/<skill_id>/.
        skill_dir = skills_dir / skill_id
        if not skill_dir.is_dir():
            skill_dir = next(skills_dir.glob(f"*/{skill_id}"), skill_dir)
        if not skill_dir.is_dir():
            mismatches.append(f"{skill_id}: not found under {skills_dir} (checked flat and one level of subfolders)")
            continue
