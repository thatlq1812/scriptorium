...
metadata:
  ...
  elicited_from: "thatlq1812 (2026-07-26): extend scouting beyond other proprietary tools, build 'office skills' from another source since proprietary docx/pdf/pptx/xlsx are license-locked. Scouting found python-docx/python-pptx/openpyxl (MIT, verified directly via pip show + the PyPI JSON API), implementation written from scratch. v0.2.0 (2026-08-01): the Vietnamese-diacritics rFonts fix and the real Word-native numbering pattern (`docx_numbering.py`) are adapted from thatlq1812's own prior production system, `D:/elix/archive/platform_archive/modules/document/v6/renderers/docx/styles.py` and `.../numbering.py` — ported and rewritten for this skill's flat JSON content spec, not blind-copied. v0.3.0 (2026-08-08): thatlq1812 directly compared a docx built by this skill against one built by the Antigravity agentic IDE for the same real essay task and found this skill's output 'không thiết kế được Docx đẹp cho lắm, chỉ đơn giản thôi' (not well-designed, too plain) -- investigation traced it to a real, named gap in this skill's own 'Known limitations' section ('no company-specific template', i.e. no cover-page capability at all), not a general styling problem; grounded in the standard Vietnamese academic/business trang bìa convention the comparison document demonstrated (institution header block, colored document-type label, colored title, label/value metadata table, closing location/date line), public-source convention, general-capability tier, no additional expert elicitation needed per principle 4. v0.3.1 (2026-08-08, same day): thatlq1812 separately flagged the actual body heading colors as 'nhạt lại còn hướng nhạt dần' (already pale AND trending paler) versus the dark navy/near-black text color expected, and explicitly directed 'có thể tự do, miễn giữ cấu trúc là được' (free to deviate on specifics as long as structure holds) rather than a rigid fixed palette -- confirmed by reading the generated style XML directly: python-docx's default template's Heading 1/2/3 theme colors run #365F91 -> #4F81BD -> #4F81BD, i.e. levels 2-3 are BOTH lighter than level 1, not darker, a real bug in the template default this skill was silently inheriting, not a style preference."
  version: 0.6.0
  changelog_0_6_0: "thatlq1812 (2026-08-14, PROJECT.md change request item 2): docx output had no default Vietnamese official-document formatting -- every document silently inherited python-docx's own US-Letter/Calibri/11pt template unless the caller happened to specify NĐ30-compliant values, which visibly hurt aesthetic output for the common case (a caller not thinking to ask for it). Fixed: create_docx.py now defaults to Nghị định 30/2020/NĐ-CP's Phụ lục I formatting -- A4 (210x297mm), Times New Roman, 13pt body text (Normal style), and NĐ30's permitted-range margins (top/bottom 20mm, left 30mm, right 15mm, the same lower-bound choice latex-project-bootstrap's vnnd30.sty already makes) -- UNLESS the content spec's top-level `font`, new `font_size`, or new `page` (`{\"width_mm\", \"height_mm\", \"margins_mm\": {\"top\"/\"bottom\"/\"left\"/\"right\"}}`) explicitly override it; `page.margins_mm` merges per-key against the NĐ30 defaults, not whole-object replacement, so a caller overriding one margin doesn't have to restate the other 3. Grounded in the same real NĐ30 PDF source already read directly for latex-project-bootstrap's vnnd30 mode (see that skill's `references/nd30-2020-formatting.md`) -- not a new elicitation, this is the general-capability formatting-drafting tier applying an already-grounded public standard to a second skill's output path. Verified real: built one .docx with no font/font_size/page fields and confirmed via python-docx read-back (`sections[0].page_width/height`, `top_margin`/etc., `styles['Normal'].font.size`, and the run-level `w:rFonts` XML) that it is exactly A4/TNR/13pt/NĐ30 margins; built a second .docx with `font: Arial`, `font_size: 11`, `page.margins_mm: {top: 25, left: 20}` and confirmed the override applied exactly (Arial/11pt, top=25/left=20 overridden, bottom=20/right=15 still NĐ30 defaults from the partial-merge)."
---
...
**Default formatting (v0.6.0): Nghị định 30/2020/NĐ-CP.** Every document defaults to Vietnamese official-document formatting — A4, Times New Roman, 13pt body text, NĐ30's permitted-range margins (top/bottom 20mm, left 30mm, right 15mm) — unless `font`, `font_size`, and/or `page` are set explicitly. This is a default, not a forced rule; a caller wanting a different look (e.g. an English-language report in Arial/Letter) sets any/all of the 3 fields and that overrides the default for exactly that field.
...
**`font` (top-level, optional, default `"Times New Roman"` per NĐ30):** applied to the `Normal` style and every `Title`/`Heading N` style actually used in `blocks`. **`font` (per-paragraph block, optional):** overrides the global font for that one paragraph's run. Both are validated against an explicit allowlist (`ALLOWED_FONTS` in `create_docx.py`: Times New Roman, Arial, Calibri, Cambria, Courier New, Georgia, Segoe UI, Tahoma, Verdana) — an unrecognized font name is refused (non-zero exit, clear error) rather than silently embedded, since an unverified font name either gets silently substituted by the renderer or renders with missing glyphs. Extend the allowlist only after confirming the font is actually available in the target rendering environment.

**`font_size` (top-level, optional, default `13`, v0.6.0):** applied in points to the `Normal` style — NĐ30 Mục V's "Nội dung" body-text row (13-14pt). Any positive number 1-96 is accepted (not restricted to the 13-14 range — a caller has real freedom to deviate, this is a default not a forced rule). Headings keep this skill's own separate default sizes, unaffected by `font_size`.

**`page` (top-level, optional object, v0.6.0):** `width_mm`/`height_mm` (default A4, 210x297) and `margins_mm` (`{"top", "bottom", "left", "right"}`, default NĐ30's permitted-range lower bound: top/bottom 20mm, left 30mm, right 15mm). `margins_mm` merges per-key against the default — overriding just `top` doesn't require restating `bottom`/`left`/`right`. Every value must be a positive number; a non-positive or missing-type value is refused loudly rather than silently falling back.
...
**v0.6.0 (2026-08-14):** `create_docx.py` now defaults to Nghị định 30/2020/NĐ-CP formatting (A4, Times New Roman, 13pt body text, NĐ30's permitted-range margins) instead of silently inheriting python-docx's US-Letter/Calibri/11pt template — unless the caller's content spec sets `font`, new `font_size`, and/or new `page` (`width_mm`/`height_mm`/`margins_mm`) explicitly. See `metadata.changelog_0_6_0` above for the full grounding and verification detail.
...
**v0.2.0 (2026-08-01):** `create_docx.py` gained two capabilities ported from thatlq1812's own prior production system (`D:/elix/archive/platform_archive/modules/document/v6/renderers/docx/`, see `metadata.elicited_from`):
