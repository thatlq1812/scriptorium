...
svg-poster-builder layout.json completely -- every zone in the layout has a
matching content entry, and vice versa (catches a typo'd zone id before it
silently drops content or crashes mid-render). No AI call, stdlib only.
    ...
    for zone_id, entry in (content.get("zones") or {}).items():
        ...
        if entry_type == "text":
            ...
            if font_path is not None:
                resolved = font_path if Path(font_path).is_absolute() else base_dir / font_path
                if not Path(resolved).is_file():
                    errors.append(f"{prefix}.font_path does not exist on disk: {resolved}")
