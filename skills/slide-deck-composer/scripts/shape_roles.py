...
def analyze_text_shape(shape, slide_width_emu: int, slide_height_emu: int) -> dict[str, Any]:
...
def collect_text_shapes(shapes, slide_width_emu: int, slide_height_emu: int) -> list[dict[str, Any]]:
    """Walk a (top-level, non-grouped) shape tree and return analyzed text shapes.

    Grouped shapes are intentionally skipped for this skill's v1 scope
    (mutating text inside a `<p:grpSp>` group correctly requires extra
    group-transform math not needed for the 6 prioritized layouts).
    """
    ...
    for shape in shapes:
        ...
        try:
            ...
            continue
        info = analyze_text_shape(shape, slide_width_emu, slide_height_emu)
        info["group_path"] = []
        out.append(info)
    return out
...
def score_picture_shape(shape, slide_width_emu: int, slide_height_emu: int) -> float:
    """Score a Picture shape as a candidate for the 'image' role of an
    LAYOUT_IMG slide: larger area and more-centered wins."""
    from pptx.enum.shapes import MSO_SHAPE_TYPE
