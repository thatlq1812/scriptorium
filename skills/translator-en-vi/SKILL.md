...
```bash
python skills/translator-en-vi/scripts/check_terminology_consistency.py usage_log.json
```
...
- Doesn't create/use a fixed terminology glossary for GENERAL translation (owner confirmed it isn't needed there) — decided contextually each time. The legal glossary above is a narrower, deliberate exception for contract/legal text specifically.
