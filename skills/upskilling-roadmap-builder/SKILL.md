...
```bash
python skills/knowledge-gap-analyzer/scripts/analyze_knowledge_gap.py <self_assessment.json> <target_requirements.json> --threshold N --json > gap_report.json
...
- Hard-coupled to `study-plan-builder`'s file layout (`skills/study-plan-builder/scripts/build_study_plan.py` relative to this skill's own script path) -- if that skill is ever renamed, moved, or removed, this skill breaks with an explicit exit-2 error naming the expected path, rather than failing silently.
