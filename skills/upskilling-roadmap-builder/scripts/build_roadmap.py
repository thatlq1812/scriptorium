...
algorithm from skills/study-plan-builder/scripts/build_study_plan.py by
...
adds gap computation on top of it. skills/study-plan-builder/ itself is
never modified.
...
def _import_scheduling_core():
    ...
    spb_scripts = Path(__file__).resolve().parents[2] / "study-plan-builder" / "scripts"
    ...
    try:
        ...
        raise RoadmapError(
            ...
            f"(expected skills/study-plan-builder/scripts/build_study_plan.py to exist as a "
            f"sibling skill): {exc}"
        ) from exc
    return spb
