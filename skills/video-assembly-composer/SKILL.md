...
description: 'Renders a final .mp4 from a plain-JSON "timeline" describing a sequence of images/video clips (with optional Ken Burns pan/zoom on images), burned-in captions, and a voice + optional ducked music-bed audio mix -- the plan/render split borrowed from OpenTimelineIO (describe the edit as data first, render second, so a bad path/duration is caught before an ffmpeg call, not during one). `validate_timeline.py` is a deterministic, stdlib-only structural validator (every video_track item''s path exists, image items require a positive duration, captions don''t overlap or exceed the track''s total length, audio paths exist, duck_under_voice requires a voice track to duck under). `render_timeline.py` builds the actual ffmpeg filter graph (via ffmpeg-python) and runs it through `ffmpeg-bootstrap`''s resolved binary -- concatenating segments, burning captions via the native `subtitles` filter (not a per-frame text overlay), and mixing/ducking audio via `sidechaincompress`. Use to assemble Gemini-generated images/video/audio (from `image-generator-gemini`/`video-generator-gemini`/`audio-generator-gemini`) into one finished video. Do NOT use this to generate any of the source images/video/audio/captions themselves -- pure assembly, no AI call of any kind.'
license: MIT
compatibility: 'Requires Python 3.11+ + `ffmpeg-python` + `imageio-ffmpeg` (shared venv) + the `ffmpeg-bootstrap` skill installed as a sibling skill folder. No AI credentials needed -- fully local, deterministic rendering. Verified running clean: Claude Code, Windows (2026-08-05), a real 3-segment render (2 Gemini-generated images with Ken Burns + 1 real Veo clip) with burned captions and a voice+ducked-music audio mix. See "Verified" below.'
metadata:
  domain: media
...
```bash
.venv\Scripts\python.exe skills\video-assembly-composer\scripts\validate_timeline.py timeline.json
```

## Render

```bash
.venv\Scripts\python.exe skills\video-assembly-composer\scripts\render_timeline.py timeline.json output.mp4
```

Builds one ffmpeg filter-graph (via `ffmpeg-python`) and runs it in a single process through `ffmpeg-bootstrap`'s `resolve_ffmpeg_path()` — every image is looped/scaled/optionally zoompan'd, every video clip is scaled+padded+letterboxed to the common output size, all segments are joined with the `concat` filter, captions (if any) are written to a temporary `.srt` next to the output and burned in via the native `subtitles` filter, and audio (if any) is mixed as described above.
